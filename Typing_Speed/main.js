...
class names {
    ...
    async signUp() {
        ...
        const signUp = this.users.find((find) => find.name === name && find.password === password && find.email === email);
        if (signUp) {
    ...
    async logIn() {
        ...
        const exist1 = this.users.find((use) => use.password === password1 && use.email === email1);
        if (exist1) {
...
async function main() {
    ...
    console.log(chalk.italic.rgb(2, 5, 7)("You will be given a text to type. Your typing speed will be measured in words per minute (WPM)."));
    ...
    console.log(chalk.bold.yellow("3. Keep your eyes on the screen and focus on the text."));
    console.log(chalk.bold.yellow("4. Type the given text as accurately and quickly as possible."));
    ...
    if (startTest) {
        console.log(chalk.italic.bold.green("<<====== Type the following text as fast as you can: ======>>"));
        ...
        console.log(chalk.italic.bold.magentaBright.underline(`\nWords per minute: ${wpm.toFixed(2)}`));
        console.log(chalk.italic.bold.red.underline(`Errors: ${errors}`));
        console.log(chalk.italic.bold.magentaBright.underline(`Accuracy: ${(100 - (errors / sampleText.length) * 100).toFixed(2)}%\n`));
    }
    else {
        console.log("Goodbye!");
    }
